...
def add_decision_variables(prog, obstacles, n_x, n_u, time_steps):
    ...
    # Slack variables
    s = prog.NewContinuousVariables(rows=time_steps + 1, cols=N_obst, name='s')
...
def set_polygon_obstacles(prog, obstacles, decision_variables, time_steps):
    ...
    for t in range(time_steps):
        ...
        for i in range(N_obst):
            ...
            prog.AddLinearConstraint( (sv >= 0.01) )
...
# ----------------------------------------------------
def add_cost(prog, decision_variables, time_interval, time_steps, kappa=10000):
    ...
    for t in range(time_steps):
        ...
        prog.AddQuadraticCost(kappa*s[t].dot(s[t]))
